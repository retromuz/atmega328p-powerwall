...
volatile byte requestedAddr = I2C_CMD_DEFAULT;
...
void loop() {
	...
	if (!mppt && loops % (LOOPS_PER_SECOND * 60l * 10l) == 0) {
		mppt = true;
	}
...
void regulate() {
	...
	if (a2 < MIN_TARGET_INPUT || input_current > MAX_INPUT_CURRENT) {
		...
		if (a3 > targetOutput || a2 < targetInput
				|| ((a3 > CHARGE_STOP_VOLTAGE_ADC - 2)
						&& output_current > MAX_OUTPUT_CURRENT_AT_SATURATION)
				|| ((a3 > CHARGE_STOP_VOLTAGE_ADC - 1)
						&& output_current > MAX_OUTPUT_CURRENT_AT_SATURATION / 2)) {
			decPWM();
...
void mpptScan(unsigned int statusesToCapture) {
	...
	} else if (loops % MPPT_STEP_DELAY_LOOPS == 0) {
...
void i2cSend() {
	...
	if (I2C_CMD_READ_TARGET_INPUT_ADC_VAL == requestedAddr) {
		byte arr[2];
		arr[0] = targetInput & 0xff;
		arr[1] = (targetInput & 0xff00) >> 8;
		Wire.write(arr, 2);
	} else if (I2C_CMD_READ_TARGET_OUTPUT_ADC_VAL == requestedAddr) {
		byte arr[2];
		arr[0] = targetOutput & 0xff;
		arr[1] = (targetOutput & 0xff00) >> 8;
		Wire.write(arr, 2);
	} else if (I2C_CMD_READ_INPUT_ADC_VAL == requestedAddr) {
		byte arr[2];
		arr[0] = a2 & 0xff;
		arr[1] = (a2 & 0xff00) >> 8;
		Wire.write(arr, 2);
	} else if (I2C_CMD_READ_OUTPUT_ADC_VAL == requestedAddr) {
		byte arr[2];
		arr[0] = a3 & 0xff;
		arr[1] = (a3 & 0xff00) >> 8;
		Wire.write(arr, 2);
	} else if (I2C_CMD_READ_OUTPUT_CURRENT_ADC_VAL == requestedAddr) {
		byte arr[2];
		arr[0] = a1 & 0xff;
		arr[1] = (a1 & 0xff00) >> 8;
		Wire.write(arr, 2);
	} else if (I2C_CMD_READ_OUTPUT_CURRENT_ESTIMATE == requestedAddr) {
		byte arr[3];
		arr[0] = output_current & 0xff;
		arr[1] = (output_current & 0xff00) >> 8;
		arr[2] = (output_current & 0xff0000) >> 16;
		Wire.write(arr, 3);
	} else if (I2C_CMD_READ_INPUT_CURRENT_ESTIMATE == requestedAddr) {
		byte arr[3];
		arr[0] = input_current & 0xff;
		arr[1] = (input_current & 0xff00) >> 8;
		arr[2] = (input_current & 0xff0000) >> 16;
		Wire.write(arr, 3);
	} else if (I2C_CMD_READ_MPPT_STATUS == requestedAddr) {
		Wire.write(noMppt ? 0 : 1);
	} else if (I2C_CMD_READ_PWM_VAL == requestedAddr) {
		Wire.write(pwm);
	} else if (I2C_CMD_READ_FREQ == requestedAddr) {
		Wire.write(TCCR2B == (_BV(WGM22) | _BV(CS20)) ? 0 : 1);
	}
	requestedAddr = I2C_CMD_DEFAULT;
}
